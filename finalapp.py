...
Data_Aggregated_Transaction_df = pd.read_csv(
    r'C:\Users\Priyabrata\Desktop\Pulse2project\Data\Agg_Trans_Table.csv')
Data_Aggregated_User_Summary_df = pd.read_csv(
    r'C:\Users\Priyabrata\Desktop\Pulse2project\Data\Agg_User_Summary_Table.csv')
Data_Aggregated_User_df = pd.read_csv(r'C:\Users\Priyabrata\Desktop\Pulse2project\Data\Agg_User_Table.csv')
Scatter_Geo_Dataset = pd.read_csv(
    r'C:\Users\Priyabrata\Desktop\Pulse2project\Data\Map_Districts_Longitude_Latitude.csv')
Coropleth_Dataset = pd.read_csv(r'C:\Users\Priyabrata\Desktop\Pulse2project\Data\Map_IndiaStates_TU.csv')
Data_Map_Transaction_df = pd.read_csv(r'C:\Users\Priyabrata\Desktop\Pulse2project\Data\Map_Transaction_Table.csv')
Data_Map_User_Table = pd.read_csv(r'C:\Users\Priyabrata\Desktop\Pulse2project\Data\Map_User_Table.csv')
Indian_States = pd.read_csv(r'C:\Users\Priyabrata\Desktop\Pulse2project\Data\Longitude_Latitude_State_Table.csv')
...
fig = px.bar(Coropleth_Dataset, x='state', y='Total_Transactions',
    ...
    with col2:
        state = st.selectbox(
            ...
            ('andaman-&-nicobar-islands', 'andhra-pradesh', 'arunachal-pradesh','assam', 'bihar', 'chandigarh', 'chhattisgarh','dadra-&-nagar-haveli-&-daman-&-diu', 'delhi', 'goa', 'gujarat',
             ...
             'nagaland', 'odisha', 'puducherry', 'punjab', 'rajasthan','sikkim', 'tamil-nadu', 'telangana', 'tripura', 'uttar-pradesh','uttarakhand', 'west-bengal'), key='b')
    State = state
    Year_List = [2018, 2019, 2020, 2021, 2022]
    Mode = mode
    State_PaymentMode = State_PaymentMode.loc[(State_PaymentMode['State'] == State) & (State_PaymentMode['Year'].isin(Year_List)) & (State_PaymentMode['Payment_Mode'] == Mode)]
...
with tab2:
    ...
    with col2:
        state = st.selectbox(
            ...
            ('andaman-&-nicobar-islands', 'andhra-pradesh', 'arunachal-pradesh','assam', 'bihar', 'chandigarh', 'chhattisgarh','dadra-&-nagar-haveli-&-daman-&-diu', 'delhi', 'goa', 'gujarat',
             'haryana', 'himachal-pradesh', 'jammu-&-kashmir','jharkhand', 'karnataka', 'kerala', 'ladakh', 'lakshadweep','madhya-pradesh', 'maharashtra', 'manipur', 'meghalaya', 'mizoram',
             'nagaland', 'odisha', 'puducherry', 'punjab', 'rajasthan','sikkim', 'tamil-nadu', 'telangana', 'tripura', 'uttar-pradesh','uttarakhand', 'west-bengal'), key='dk')
    with col3:
        Quarter = st.selectbox(
...
with tab3:
    ...
    Year = int(Y)
    Mode = M
    Year_PaymentMode = Year_PaymentMode.loc[(Year_PaymentMode['Year'] == Year) & (Year_PaymentMode['Payment_Mode'] == Mode)]
    ...
    fig2 = px.bar(Year_PaymentMode_Table, x='states', y='Total_Transactions_count', color="Total_Transactions_count",color_continuous_scale="Viridis",)
    colT1, colT2 = st.columns([7, 3])
    with colT1:
...
with tab4:
    ...
    with col2:
        ...
        st.markdown(years_Table.style.hide(
...
with tab1:
    ...
    state = st.selectbox(
        ...
        ('andaman-&-nicobar-islands', 'andhra-pradesh', 'arunachal-pradesh','assam', 'bihar', 'chandigarh', 'chhattisgarh','dadra-&-nagar-haveli-&-daman-&-diu', 'delhi', 'goa', 'gujarat',
             'haryana', 'himachal-pradesh', 'jammu-&-kashmir','jharkhand', 'karnataka', 'kerala', 'ladakh', 'lakshadweep','madhya-pradesh', 'maharashtra', 'manipur', 'meghalaya', 'mizoram',
             'nagaland', 'odisha', 'puducherry', 'punjab', 'rajasthan','sikkim', 'tamil-nadu', 'telangana', 'tripura', 'uttar-pradesh','uttarakhand', 'west-bengal'), key='W')
    ...
    fig = go.Figure(data=[
        go.Bar(name='AppOpenings %', y=sout['AppOpenings'],
               x=sout['state_year'], marker={'color': 'pink'}),
...
with tab3:
    ...
    fig3 = go.Figure(data=[go.Pie(labels=labels, values=values, hole=.4, textinfo='label+percent',
                     texttemplate='%{label}<br>%{percent:1%f}', insidetextorientation='horizontal', textfont=dict(color='#000000'), marker_colors=px.colors.qualitative.Prism)])
...
with col2:
    ...
    st.markdown(at[['State', 'AppOpenings']].style.hide(
        axis="index").to_html(), unsafe_allow_html=True)
with col3:
